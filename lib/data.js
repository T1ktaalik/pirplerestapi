...
lib.update = function(dir, file, data, callback) {
  ...
  fs.open(lib.baseDir + dir + '/' + file + '.json', 'r+', function(err, fileDescriptor){if(!err){
    //Convert data to string
    var stringData = JSON.stringify(data);

    //Truncate the file
    fs.ftruncate(fileDescriptor, function(err){if(!err){
      //write to the file and close it may
      fs.writeFile(fileDescriptor, stringData, function(err){
        if(!err){fs.close(fileDescriptor, function(err){
          if(!err){callback(false)}else{'Error closing the file'}
        })}
      else{callback('error writing the existing file')}} )
    }else{callback('error truncating file')}})
  }else{callback('could not open the file it may not exist yet')}} )
}
// Deleting a file
